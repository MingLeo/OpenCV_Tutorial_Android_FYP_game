...
public class IdentifyCornerOfInputScreen_Flags extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    //JavaCameraView javaCameraView;
//    CameraBridgeViewBase cameraBridgeViewBase;
    customSurfaceView cameraBridgeViewBase;    //cameraBridgeViewBase change to instance of customSurfaceView class to access both Camera mCamera object + CameraBridgeViewBase class!
...
//        //// ===================== We Use timer.shedule() here which is much more simpler, no nd handler, looper, etc.... :) :D ===============================
//
...
//
//        final ArrayList<Rect> rect_array = new ArrayList<Rect>();
//        rect_array.add(r1);
//        rect_array.add(r2);
//        rect_array.add(r3);
//        rect_array.add(r4);
...
//            final Rect r = rect_array.get(i);
//
//
////            new CountDownTimer(30000, 5000) {      // RuntimeException: Can't create handler inside thread that has not called Looper.prepare()
////                @Override
////                public void onTick(long millisUntilFinished) {
////                    Imgproc.rectangle(InputFrame, r.tl(), r.br(), new Scalar(0, 255, 0), 4);
////                    Log.d("timer Schedule for", "r:" + r);
...
//            timer.scheduleAtFixedRate(new TimerTask() {   //https://stackoverflow.com/questions/4993765/how-to-stop-the-timer-in-android
//                //https://developer.android.com/reference/java/util/Timer.html#scheduleAtFixedRate(java.util.TimerTask,%20long,%20long) - Schedules the specified task for repeated fixed-rate execution
//
//                int secs = 4;
//
//                @Override
...
//                    Imgproc.rectangle(InputFrame, r.tl(), r.br(), new Scalar(0, 255, 0),4);
//                    Log.d("timer Schedule for","r:"+r);  //Logcat output shows that it actl prints all 4 at once, which is not what we want! We want it to print/appear 4 seconds apart.
//
////                    //add a delay here for 4s, just do nothing but purely waste 4 secs of life here!
////                    Utils.delay(secs, new Utils.DelayCallback() {     // RuntimeException: Can't create handler inside thread that has not called Looper.prepare()
////                        @Override
...
//
//                }
//            }, 0,4000);     //task : the task to schedule
//            //delay: amount of time in milliseconds before first execution.
//            //period: amount of time in milliseconds between subsequent executions.
//
//            SystemClock.sleep(4000);
//            Log.d("Executing SystemClock sleep: ","4000");    //unable to return anyting for input frame cos i pause the UI Thread for too long.
//        }
//        //remarks: I think i have misconception of delay: delay here is time dealyed bef execution, period is btwn successive task,
//        //BUT NONE OF THEM HANDLES DURING THE TASK, SO I WANT TO FREEZE THERE FOR 4secs, ah, see they dun have
//        //then plus see also https://developer.android.com/reference/java/util/Timer.html#scheduleAtFixedRate(java.util.TimerTask,%20long,%20long)
//        //says that if got GC garbage collection, two or more executions will occur in rapid succession to "catch up.", that's why in the end all appear tgt at the same time.
//
//////======================================================================================================================================
...
//================ Code Logic for controlling Camera Frame Rate via custom method  setPreviewFPS() in our CustomSurfaceView class, if we nd to deliberately set/control the FpsRange() FrameRate .  But actl now that we do away w Thread in onCameraFrame & replace w flags for the drawing of the boundingRect, might want to speed up the Frame Rate so as to make it less Laggy!   Either set to Default 24 fps / set to index(3) at 30 frames per sec. =============================
...
// frame==== //@@ https://stackoverflow.com/questions/22456817/how-to-limit-preview-fps-range-in-android-for-camera - Set FrameRate FpsRange
        Camera.Parameters l_params = cameraBridgeViewBase.getParameters();    //cameraBridgeViewBase change to instance of customSurfaceView class to access both Camera mCamera object + CameraBridgeViewBase class!
        ...
        Log.d("print","framerates" + frameRates);    //prints out gibberish, think is Int Array[] , only 4 supported FPSRange/FrameRate:
                                                               // [I@427f4d, [I@1a7da02, [I@8709f13, [I@41db550].
                                                               //(15000,15000), (24000,24000), (15000,30000), (30000,30000) Ideally shld look something understandable like (10000,10000),(15000,15000),(15000,30000),(30000,30000) instead.
...
//            }
        ...
        //see logcat of framerates, only 4 supported FPSRange, test all 4, indexing (0-3), frameRates.get(0-3).
        int l_first = 0;    //index of the lowest possible frame rate of the frameRates array.
        int l_last = frameRates.size() - 1;
        int minFps = (frameRates.get(3))[PREVIEW_FPS_MIN_INDEX];   //set to the min frame of the lowest poss framte rate at index(0) of all supported FPSRange size!
        int maxFps = (frameRates.get(3))[Camera.Parameters.PREVIEW_FPS_MAX_INDEX];   //set to the max frame of the lowest poss framte rate at index(0) of all supported FPSRange size!
        //frameRate FPSRange for my Samsung s7 mobile device which I indiv test & confirm alrdy!
...
// screen size== //https://stackoverflow.com/questions/8744994/android-camera-set-resolution - set Camera Screen Resolution.
        ...
        for (Camera.Size size : screenResolutionSizes) {
            Log.d("List of","available screenResolution size, width, height:" + size.width+ " " + size.height);
                //(2592,1944), (2560,1440), (2048,1536), (1936,1936), (1920,1440), (1920,1080), (1440,1440), (1280,720), (960,720), (620,480), (352,288), (320,240), (256,144)  Resolution screen sizes in descending order.
...
//        Scalar color = new Scalar(0,255,0);   //r,g,b, so only show green box
   ...
   ////=====================  =D WORKED! DREW 4 floodfilled RECTANGLES ON THE 4 CORNERS OF THE SCREEN! ==============================
...
//            Rect r3 = new Rect(0,772,300,300);
        ...
        if(flag4 == true){
            ...
            final Mat maskFloodFill = Mat.zeros(new Size(InputFrame.cols() + 2, InputFrame.rows() + 2), CvType.CV_8UC1);
//            Rect r = rect_array.get(3);
//            Rect r4 = new Rect(772,772,300,300);
            Imgproc.rectangle(InputFrame, r4.tl(), r4.br(), new Scalar(0, 255, 0), 4);    //top right corner
            Imgproc.floodFill(InputFrame, maskFloodFill,new Point((r4.tl().x +r4.br().x) / 2, (r4.tl().y +r4.br().y) / 2), new Scalar(0,255,0), new Rect() , new Scalar(20,20,20),new Scalar(20,20,20),4);
